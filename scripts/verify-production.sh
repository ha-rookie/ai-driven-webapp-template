...
if [ "$REQUIRE_SECURITY_HEADERS" = "true" ]; then
  ...
  grep -Eqi '^x-content-type-options:[[:space:]]*nosniff[[:space:]]*fi
...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
 ...
 "$headers_lower" || {
  ...
  grep -Eqi '^x-permitted-cross-domain-policies:[[:space:]]*none[[:space:]]*fi
...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
 ...
 "$headers_lower" || {
  ...
  if grep -qi '^x-frame-options:' "$headers_lower"; then
    grep -Eqi '^x-frame-options:[[:space:]]*(deny|sameorigin)[[:space:]]*fi
